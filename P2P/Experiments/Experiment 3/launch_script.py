...
def main():
    ...
    parser.add_argument("number_of_threads", type=int, help="Number of buyers")
    args = parser.parse_args()

    N = args.number_of_nodes
    threads = args.number_of_threads
    exp_file_name = "metrics_threads" + str(threads) + ".txt"
    ...
    for idx, ip in enumerate(range(INITIAL_IP, last_ip + 1), start=1):
        ...
        subprocess.run([
            ...
            f"'{neighbor_ip}' '{is_source}' '{secret}' '{query}' '{N-1}' '{exp_file_name}' '{threads}'"
        ], check=True)
