...
void start_client(string ID, string filename){
	fstream file(filename, ios::in | ios::out | ios::trunc);
	if(! file ){
		cerr<<"[ERROR] : Client "<<ID<<" Error opening file "<<strerror(errno)<<endl;
	}
	if(config.IS_SOURCE){
		int knt = 0;
		int success_knt = 0;
		while(true){
			...
			auto time_request_sent = chrono::high_resolution_clock::now(); 
			string response = client.send_request(&query_message);
			auto time_request_received = chrono::high_resolution_clock::now();
			auto duration = chrono::duration_cast<chrono::milliseconds>(time_request_received - time_request_sent);
			cout<<"[METRICS] : Client "<<ID<<" Response time - request -"<<knt<<" "<<duration.count()<<" in ms";
			string output = "[RESPONSE_TIME] Request-" + to_string(knt) + " " + to_string(duration.count());
			file << output <<endl;
			...
			int random_idx = rand() % (ITEMS.size());
			config.QUERY_STRING =  ITEMS[random_idx];
			cout<<"[INFO] : Client "<<ID<<" Cycle completed starting new cycle \n \n \n"<<endl;

			//this_thread::sleep_for(chrono::seconds(10));
			if(knt == 1000){
				cout<<"[INFO] : Client "<<ID<<" exiting"<<endl;
				cout<<"[METRICS] : Client "<<ID<<" Successful hits: "<<success_knt<<endl;
				string output = "[HIT_RATE] Requests-1000," + to_string(knt);
				file << output <<endl;
				file.close();
				break;
			}
...
int main(int argc, char* argv[]){
	if(argc < 13){
		cout<<
			...
			"<IS_SOURCE:BOOL> <SECRET> <QUERY_STRING> <HOP_COUNT> <EXP_FILE_NAME>, Please provide all the required arguments to start the server."
			<<endl;
		return -1;
	}
	...
	thread client(start_client, id, filename);
