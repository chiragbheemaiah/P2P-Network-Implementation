#!/bin/bash
CONSOLIDATED_FILE="results.txt"
for(( threads=0; threads<=4; threads++ ))do
	NUM_CONTAINERS=10
	OUTPUT_FILE="Final_metrics_$((2 ** threads)).txt"
    INPUT_USAGE_FILE="usage_threads_$((2 ** threads)).log"
    INPUT_METRIC_FILE="metrics_threads$((2 ** threads)).txt"
	...
	awk '$2 == "CPU" {sum += $5} END {print "Overall CPU Utilization: " sum/NR }' $OUTPUT_FILE >> "$CONSOLIDATED_FILE"	
	awk '$2 == "Memory" {sum += $5} END {print "Overall Memory Utilization: " sum/NR }' $OUTPUT_FILE >> "$CONSOLIDATED_FILE"
	awk '$3 == "Input" {sum += $5} END {print "Overall Network Input Utilization: " sum/NR }' $OUTPUT_FILE >> "$CONSOLIDATED_FILE"
	awk '$3 == "Output" {sum += $5} END {print "Overall Network Output Utilization: " sum/NR }' $OUTPUT_FILE >> "$CONSOLIDATED_FILE"
	
	awk '$1=="[RESPONSE_TIME]" && $3 ~ /^[0-9]+(\.[0-9]+)?$/ { sum += $3; n++ } \
 	END { if (n>0) printf "Average Response Time: %.2f\n", sum/n }' \
    "$INPUT_METRIC_FILE" >> "$CONSOLIDATED_FILE"
